...
export async function docentToewijzen(positionId: string, teacherId: string, opties?: { reserve?: boolean; conflictReden?: string }) {
  const u = await vereisRol(...PLANNEN);
  ...
  const bezet = pos.assignments.filter((a) => !a.reserve && !a.uitgevallen).length;
  if (!opties?.reserve && bezet >= pos.aantal) return { fout: "Deze positie is al volledig bezet." };

  const t = await db.teacherProfile.findUnique({
    where: { id: teacherId },
    include: { user: true, assignments: { include: { position: { include: { session: true } } } } },
  });
  if (!t) return { fout: "Docent niet gevonden." };
  if (t.status !== "GOEDGEKEURD") return { fout: "Deze docent is nog niet goedgekeurd." };
  ...
  if (conflict && !opties?.conflictReden) {
    return { fout: "Deze docent heeft op dit tijdstip al een opdracht. Geef een reden op om dit toch door te zetten.", conflict: true };
  }

  await db.$transaction(async (tx) => {
    await tx.assignment.create({ data: { positionId, teacherId, reserve: opties?.reserve ?? false } });
    await tx.application.updateMany({
      where: { positionId, teacherId },
      data: { status: opties?.reserve ? "RESERVELIJST" : "GESELECTEERD", gereageerdOp: new Date() },
    });
  });
  ...
  await logAudit({
    userId: u.id,
    actie: opties?.reserve ? "RESERVE_TOEGEWEZEN" : "DOCENT_TOEGEWEZEN",
    entiteit: "StaffingPosition",
    entiteitId: positionId,
    nieuw: { teacherId, conflictReden: opties?.conflictReden ?? null },
    ip: ipAdres(),
  });
  revalidatePath(`/beheer/opdrachten/${pos.sessionId}`);
  return { ok: true };
...
export async function toewijzingIntrekken(assignmentId: string, reden: string) {
  ...
  revalidatePath(`/beheer/opdrachten/${a.position.sessionId}`);
  return { ok: true };
}

export async function aanmeldingStatus(applicationId: string, status: "IN_BEHANDELING" | "RESERVELIJST" | "AFGEWEZEN", notitie?: string) {
  const u = await vereisRol(...PLANNEN);
  ...
  if (status === "AFGEWEZEN" || status === "RESERVELIJST") {
    await meld({
      userId: a.teacher.userId,
      email: a.teacher.user.email,
      sleutel: status === "AFGEWEZEN" ? "AFGEWEZEN" : "RESERVELIJST",
