  ...
  return (
    <div>
      <div className="rounded-xl border border-skool-200 bg-skool-50 p-4">
        ...
        <p className="mt-1 text-sm text-zand-600">
          ...
          return (
            <div key={r.id} className="kaart p-4">
              <div className="flex items-baseline justify-between gap-3">
                <div className="text-sm font-medium text-zand-500">
                  {r.startTijd} tot {r.eindTijd}
                </div>
                <div className="text-xs text-zand-400">Ronde {r.nummer}</div>
              </div>
              <div className="mt-1 text-lg font-semibold">{slot?.workshop ?? "Nog niet gekozen"}</div>
              {slot?.ruimte && <div className="text-sm text-zand-500">{slot.ruimte}</div>}
            </div>
          );
        })}
      </div>
