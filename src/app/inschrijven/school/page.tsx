  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-neutral-500">{e.project.client.naam}</p>
      </div>
      ...
      <Kaart>
        ...
        <p className="mt-2 text-sm text-neutral-500">{klaar} van {totaal} leerlingen zijn klaar.</p>
      </Kaart>

      <Kaart>
        <h2 className="font-semibold">Nog niet ingeschreven</h2>
        {open.length === 0 ? (
          <p className="mt-2 text-sm text-neutral-500">Iedereen is klaar. Mooi.</p>
        ) : (
          <>
            ...
            <ul className="mt-3 max-h-64 space-y-1 overflow-y-auto text-sm">
              {open.map((d) => (
                <li key={d.id} className="flex justify-between border-b border-neutral-100 py-1">
                  <span>{[d.voornaam, d.achternaam].filter(Boolean).join(" ")}</span>
                  <span className="text-neutral-400">{d.klas}</span>
                </li>
              ))}
            </ul>
          </>
                  ...
                  return (
                    <div key={s.id} className="text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-neutral-500">{s._count.keuzes} van {s.capaciteit}</span>
                      </div>
