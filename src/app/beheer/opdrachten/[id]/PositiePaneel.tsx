...
type Toewijzing = { id: string; naam: string; email: string; telefoon: string | null; reserve: boolean; bevestigd: boolean; uitgevallen: boolean };
...
export default function PositiePaneel({
  ...
  const bezet = toewijzingen.filter((t) => !t.reserve && !t.uitgevallen).length;
    ...
    <div className="kaart p-4 sm:p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-neutral-500">{bezet} van {positie.aantal} bezet · {euro(positie.vergoeding)} per docent</p>
        </div>
      ...
      {/* Toegewezen docenten */}
      <h3 className="mb-2 text-sm font-semibold uppercase tracking-wide text-neutral-500">Toegewezen</h3>
      {toewijzingen.length === 0 ? (
        <p className="mb-4 text-sm text-neutral-500">Nog niemand toegewezen.</p>
      ) : (
        <ul className="mb-4 space-y-2">
          {toewijzingen.map((t) => (
            <li key={t.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-sm">
              <span className="font-medium">{t.naam}</span>
              <span className="text-neutral-500">{t.email}</span>
              {t.reserve && <Badge kleur="paars">Reserve</Badge>}
              {t.uitgevallen && <Badge kleur="rood">Uitgevallen</Badge>}
              {t.bevestigd ? <Badge kleur="groen">Bevestigd</Badge> : <Badge kleur="geel">Nog niet bevestigd</Badge>}
      ...
      <h3 className="mb-2 text-sm font-semibold uppercase tracking-wide text-neutral-500">Aanmeldingen en uitnodigingen</h3>
      {aanmeldingen.length === 0 ? (
        <p className="mb-4 text-sm text-neutral-500">Nog geen reacties.</p>
      ) : (
        <ul className="mb-4 space-y-2">
          {aanmeldingen.map((a) => (
            <li key={a.id} className="rounded-lg border border-neutral-200 px-3 py-2 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">{a.naam}</span>
                <Badge kleur={statusKleur(a.status)}>{label(a.status)}</Badge>
                <span className="text-xs text-neutral-500">{a.soort === "UITNODIGING" ? "directe uitnodiging" : "eigen aanmelding"}</span>
                {a.deadline && <span className="text-xs text-neutral-500">reageren voor {a.deadline}</span>}
              </div>
              {a.motivatie && <p className="mt-1 text-neutral-600">{a.motivatie}</p>}
              {magPlannen && ["AANGEMELD", "IN_BEHANDELING", "BEKEKEN"].includes(a.status) && (
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <button className="knop-secundair px-3 text-xs" disabled={bezig}
                    onClick={() => doe(() => aanmeldingStatus(a.id, "RESERVELIJST"), "Op de reservelijst gezet.")}>
                    Reservelijst
                  </button>
                  <button className="knop-secundair px-3 text-xs" disabled={bezig}
      ...
      {magPlannen && toonMatch && (
        <>
          ...
          <ul className="space-y-2">
            {matches.map((m) => (
              <li key={m.teacherId} className="rounded-lg border border-neutral-200 p-3 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    type="checkbox"
                    className="h-4 w-4"
                  ...
                  {m.afstand !== null && <span className="text-xs text-neutral-500">{m.afstand} km · ongeveer {m.reistijd} min</span>}
                  {m.eerderVoorKlant > 0 && <span className="text-xs text-neutral-500">{m.eerderVoorKlant}x eerder bij deze klant</span>}
            ...
            {matches.length === 0 && <li className="text-sm text-neutral-500">Geen geschikte docenten gevonden.</li>}
          </ul>
          {gekozen.length > 0 && (
