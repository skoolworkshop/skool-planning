  ...
  if (vergeten) {
    return (
      <form action={resetActie} className="kaart space-y-4 p-5">
        ...
        <p className="text-sm text-zand-500">Vul je e-mailadres in. Je krijgt een link om een nieuw wachtwoord in te stellen.</p>
        <div>
        ...
        {resetState.fout && <p className="text-sm text-zand-600">{resetState.fout}</p>}
  ...
  return (
    <form action={actie} className="kaart space-y-4 p-5">
      ...
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" name="onthoud" className="h-4 w-4 rounded border-zand-300" />
